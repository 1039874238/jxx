...
class Bots extends Service {
    ...
    async addStudents(params) {
        ...
        newUserList.forEach(item => {
            let currentBrowser = browser[currentGroup]
    ...
    async checkBrowser() {
        let browsers = await this.ctx.model.BotBrowser.find({ status: '1' })
        if (browsers.length > 0) {
            const needNotice = []
            for (let index = 0; index < browsers.length; index++) {
                if (dayjs().diff(dayjs(browsers[index].runTime), 'minutes') > 3) {
    ...
    async getBot(params) {
        ...
        for (let index = 0; index < bot.length; index++) {
            let browser = await this.ctx.model.BotBrowser.find({ botName: bot[index].botName })
            output.push({
    ...
    async getConfig() {
        let output = await this.ctx.model.BotConfig.find()
        this.ctx.body = {
            state: 200,
            data: output,
            msg: '成功',
